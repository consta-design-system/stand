...
const getMenuData: GetMenuData = (children) => {
    const { props: { children: childrenProp, href } } = children;
...
export const useMenu: UseMenu = () => {
    ...
    const showMenu = useMemo(() => {
        const params ={ libId: stand?.lib.id, standId: stand?.id };
        return (
          !!menuNode
          || router.isActive(routesNames.LIBS_LIB_STAND, params)
          || router.isActive(routesNames.LIBS_LIB_STAND_DEV, params)
          || router.isActive(routesNames.LIBS_LIB_STAND_DESIGN, params)
          || router.isActive(routesNames.LIBS_LIB_STAND_SANDBOX, params)
        );
    }, [menuNode, stand, path])
